...
const clientAssetsDir = join(appDir, '.output', 'public', '_nuxt');
const outputDir =
	process.env.BENCH_OUTPUT_DIR ?? '../../.benchmarks/current/bundle-nuxt';
...
> {
	...
	return await Promise.all(
		assetEntries.map(async (entry) => {
			...
			const type = extname(entry.name).slice(1) as ClientAssetSize['type'];
...
const main = async function main() {
	if (!existsSync(clientAssetsDir)) {
		throw new Error(
			`Nuxt client assets were not found at ${clientAssetsDir}. Run "bun run build" first.`
		);
	}
	...
	const result: BenchmarkResult = {
		...
		notes: [
			'Totals include every emitted .js and .css file in .output/public/_nuxt.',
			'Route entry graphs are omitted because this build has no .nuxt/dist/client/manifest.json and its published build metadata contains no chunk graph.',
		],
		package: '@c15t/nuxt-browser-bench',
		runtime: 'nuxt',
		scenario: 'nuxt-client-bundle',
