...
export const createIABSurface = (
	...
	const build = (snapshot: ConsentSnapshot, dialog: boolean): void => {
		...
		const title = dialog ? t.preferenceCenter.title : t.banner.title;
		const content = h('div', {
			'aria-labelledby': `${prefix}-title`,
			'aria-modal': actions.blocking ? 'true' : undefined,
			class: css(styles.card),
			'data-testid': `${prefix}-card`,
			role: actions.blocking ? 'dialog' : 'region',
			tabindex: '-1',
		});
		const headerCopy = h(
			'div',
		...
		headerCopy.append(
			...renderLegalLinks({
				keys: ['privacyPolicy', 'cookiePolicy'],
		...
		if (dialog) {
			...
			root = h(
				'div',
				{
					...
					'data-position': actions.position,
					'data-testid': `${prefix}-root`,
