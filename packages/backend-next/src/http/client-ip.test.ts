...
// implementations.
import { getIpAddress as shipped } from '../../../backend/src/middleware/process-ip';
...
describe('parity with @c15t/backend', () => {
	it('agrees across headers, chains and configurations', () => {
		const cases: Array<Record<string, string>> = [
			...
			{ 'user-agent': 'none' },
			// Precedence: x-client-ip is checked before x-forwarded-for.
			{ 'x-client-ip': '198.51.100.5', 'x-forwarded-for': '203.0.113.42' },
		];
		const configs = [
			undefined,
			{ tracking: false },
		...
		for (const headers of cases) {
			for (const config of configs) {
				const h = headersWith(headers);
				assert.strictEqual(
					getIpAddress(h, config),
					shipped(h, { ipAddress: config } as never),
					`${JSON.stringify(headers)} / ${JSON.stringify(config)}`
				);
			}
		}
	});
