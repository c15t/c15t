...
import type { ConsentConfig as InitialConsentConfig } from '../server';
...
export const createPolicySession: CreatePolicySession = async (setup) => {
	...
	let serverConfig: InitialConsentConfig | undefined;
	let scriptLoads = 0;
	...
	const mount = async (hydrate = false) => {
		...
		if (hydrate) {
			...
			// A browser Request strips Cookie; the server Request retains it.
			Object.defineProperty(request, 'headers', {
				value: new Headers({ cookie }),
			});
			serverConfig = await readInitialConsentConfig({
				cookieName: config.storageKey,
				now: setup.clock.now(),
				request,
			});
			const app = tree(true);
