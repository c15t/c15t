...
const generateUiCssAssets = function generateUiCssAssets(missingPath: string) {
...
await Array.from(inlinedFiles).reduce(
	async (previousIteration, relativePath) => {
		...
		const header = [
			'/**',
			' * @c15t/tanstack-start - Tailwind 3-compatible prebuilt component styles.',
			' *',
