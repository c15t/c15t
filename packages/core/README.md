...
1. Create a kernel with a transport for your deployment mode
2. Mount the side-effect modules you need (persistence, script loader, blockers)
3. Read consent through `getSnapshot()` / `subscribe()` and write through `set.*` / `commands.*`
4. For full implementation details, see the [JavaScript quickstart docs](https://c15t.com/docs/frameworks/javascript/quickstart)

```typescript
import { createConsentKernel, hosted } from '@c15t/core'
import { createPersistence } from '@c15t/core/modules/persistence'

const kernel = createConsentKernel({
  transport: hosted({ url: 'https://your-instance.c15t.dev' })(),
})
