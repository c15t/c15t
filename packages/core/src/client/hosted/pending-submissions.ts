 ...
 * @internal
 */
export const checkPendingConsentSubmissions =
	function checkPendingConsentSubmissions(
		...
		try {
			...
			setTimeout(() => {
				...
				// Delay to ensure page is loaded and network is likely available
			}, 2000);
		} catch (error) {
 ...
 * @internal
 */
export const processPendingConsentSubmissions =
	async function processPendingConsentSubmissions(
 ...
 * @internal
 */
export const checkPendingIdentifySubmissions =
	function checkPendingIdentifySubmissions(
		_context: FetcherContext,
		processPendingSubmissions: (
		...
		try {
			...
			setTimeout(() => {
				processPendingSubmissions(pendingSubmissions);
				// Delay slightly longer than consent to avoid race conditions
			}, 2500);
		} catch (error) {
			// Ignore localStorage errors but log them
 ...
 * @internal
 */
export const processPendingIdentifySubmissions =
	async function processPendingIdentifySubmissions(
		context: FetcherContext,
		submissions: IdentifyUserRequestBody[]
	) {
		const maxRetries = 3;
		...
		try {
			...
			console.warn(
				'Error updating pending identify submissions storage:',
				error
			);
		}
	};
