...
* For iframe-only embeds, gate the iframe `src` with the [iframe blocking](/docs/frameworks/react/components/frame) pattern instead of loading a script just to hide an iframe.
