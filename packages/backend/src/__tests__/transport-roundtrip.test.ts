...
describe('backend producer, hosted transport, mapper and evaluator', () => {
	...
	it('reads a save made by a client without receipts as legacy receipts the evaluator grandfathers', async () => {
		...
		const legacyClient = createHostedTransport({
			backendURL: 'https://backend.test/c15t',
			domain: 'example.com',
			fetch: ((input: string | URL | Request, init?: RequestInit) =>
				harness.app.request(
		...
		// A kernel that predates receipts: no `choice`, no `confirmed`.
		const saved = await legacyClient.save({
			consentAction: 'custom',
			consents: {
				experience: false,
				functionality: false,
		...
		const records = await legacyClient.loadSubjectRecord(subjectId);
