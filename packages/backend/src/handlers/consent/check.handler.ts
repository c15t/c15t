/**
 * GET /consent/check handler - Pre-banner cross-device consent check.
 *
 ...
 */
export const checkConsent = os.consent.check.handler(
	async ({ input, context }) => {
		const typedContext = context as C15TContext;
		const logger = typedContext.logger;
		logger.info('Handling GET /consent/check request');
		...
		if (!type) {
			throw new ORPCError('TYPE_REQUIRED', {
				data: {
		...
		try {
			...
			logger.error('Error in GET /consent/check handler', {
