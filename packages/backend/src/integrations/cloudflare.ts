...
export function toCloudflareHandler(instance: C15TInstance) {
   ...
   * Handle CORS preflight requests directly
   */
  function handleCorsPreflightRequest(request: Request, instance: C15TInstance): Response {
   ...
   */
  function addCorsHeaders(response: Response, request: Request, instance: C15TInstance): Response {
    const origin = request.headers.get('Origin');
    const isTrusted = isTrustedOrigin(origin, instance);
   ...
   */
  function isTrustedOrigin(origin: string | null, instance: C15TInstance): boolean {
