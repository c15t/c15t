...
/** Writes framework stylesheet entrypoints using the shared UI CSS assets. */
export const generateDistributionCss = (packageRoot: string): void => {
	...
	const formatCommandOutput = function formatCommandOutput(
		label: string,
		output?: string | null
	) {
	...
	const generateUiCssAssets = function generateUiCssAssets(
		missingPath: string
	) {
		...
		if (result.status !== 0) {
			...
			throw new Error(
				`Failed to generate @c15t/ui CSS entrypoints with ${exitReason}.${formatCommandOutput(
					'stdout',
					result.stdout
	...
	const getUiCssAsset = function getUiCssAsset(relativePath: string) {
	...
	for (const relativePath of inlinedFiles) {
		...
		const header = [
			'/**',
			` * ${packageName} - Tailwind 3-compatible prebuilt component styles.`,
			' *',
		...
		writeFileSync(targetPath, `${header}${sourceCss}`);
	}
