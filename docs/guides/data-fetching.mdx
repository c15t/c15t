...
a server, use a cached manifest to resolve policy in your application and keep
consent submissions going to Inth. Follow the
[Next.js manifest setup](/docs/frameworks/next/data-fetching).

Backend ownership and data fetching are separate decisions. A manifest works
with Inth or a self-hosted c15t backend. A static site can still call Inth. Only
`offline()` deliberately removes consent backend requests.
...
| Offline | The browser or local runtime, using bundled rules | No backend submission | Demos, tests or an application that deliberately needs only local records |
...
A same-origin URL named `/api/consent/init` can instead resolve from a cached
...
For a custom same-origin init route that resolves a manifest, the hosted
transport can separate reads and writes:
...
The init route does not proxy consent writes. Either allow the browser origin
at the backend, or configure an actual proxy for the backend URL, including the
record endpoints. A static export cannot serve a Next.js route or rewrite at
runtime.
...
page requests do not call the backend `/init`, a visitor's choice still posts to
`/subjects`, and policy changes become visible after the configured refresh.
