...
browser consent traffic through `/api/c15t` on your app's origin, and use a
Next.js rewrite to forward record requests to Inth. Follow the
[Next.js manifest and rewrite setup](/docs/frameworks/next/data-fetching).
...
and other record endpoints. Configure both. The
[Next.js router guides](/docs/frameworks/next/quickstart) use `consentConfig`
from `c15t.config.ts` to select this transport, so their boundary does not need
a separate `mode` override.

Keeping c15t requests on the app's origin avoids a separate browser DNS lookup
