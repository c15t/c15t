...
forwarded headers under the backend's usual IP handling. One page view can
produce a `render` and a `route` report, so count on the consuming side by
deduplicating on `source`. Static output resolves in the browser and sends none. Set `reportSessions: false` on an adapter to turn
it off.
