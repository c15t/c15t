...
The stylesheet holds every component rule, the default tokens and the shared
primitive styles. React, Next.js, TanStack Start and Svelte components do not
load CSS themselves, so each rule reaches the page once, from this file. A
component rendered without it is unstyled.
