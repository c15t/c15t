...
`presentation` to `options.presentation` on the boundary. `ConsentBanner` has
no `classNames` prop: put per-slot class names in
`options.components.banner.<slot>.className`, or compose the banner from its
compound parts when you need different markup.
