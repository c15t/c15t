...
`ConsentBanner` has no `classNames` prop. Move each legacy key to the boundary
`options.components` slots, which take `{ className }`:
...
| `acceptButton`, `rejectButton`, `customizeButton`, `dismissButton` | No per-button slot. Use `components.button.primary` / `.secondary` for shared styles, or target `[data-action="accept"]`, `"reject"`, `"customize"`, `"dismiss"` in CSS. For different markup, compose `ConsentBanner` from its compound parts. |
