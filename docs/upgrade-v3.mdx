...
Replace `<RscConsentBanner config={config} />` with `<ConsentBanner />` from
`c15t/next` inside the same `ConsentBoundary`. With an awaited
`prefetchInitialConsent` result, the banner is already part of the first HTML
response, which is what the RSC variant was for. Props such as `classNames`
and `presentation` move to `ConsentBanner` props and `options.presentation`.
