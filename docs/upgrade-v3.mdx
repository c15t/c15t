...
`prompt: 'choice'`. Opt-out supports `choice`, `notice`, and `none`.

A strict scope blocks categories outside the rule. A permissive scope allows
...
Use an explicit fallback rule for unknown locations. Resolution exposes
`matched`, `no-match`, `unconfigured`, or `failed`; non-matched outcomes use the
engine's conservative fallback. A missing or malformed policy never grants
