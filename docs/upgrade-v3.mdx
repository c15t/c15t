...
Two defaults differ from the removed shell. It rendered no branding link and
applied no base classes; `ConsentBanner` shows the "Secured by c15t" link
unless you pass `hideBranding`, and merges the stock styles into every slot
unless you pass `noStyle`. For visual parity start from
`<ConsentBanner hideBranding noStyle />` and remove the props you want.
...
`FooterSubGroup` and the four buttons are available for finer control. Place
the former `children` inside `ConsentBanner.Card`:
