...
One transform targets v3: `use-consent-manager-to-hooks` rewrites `useConsentManager()` destructuring to the v3 hooks. It runs only when named, never with `--all`:

```bash
c15t codemods use-consent-manager-to-hooks --dry-run --json
```

Fields with no one-hook equivalent stay on a `useConsentManager()` call under a `TODO(c15t v3)` comment, so the build fails where manual work remains. The [upgrade guide](/docs/upgrade-v3#replace-useconsentmanager) maps every field.
