...
the background. On a runtime that stops work after the response is sent, pass
`onBackgroundRevalidate` to the handlers and hand the promise to your
platform's `waitUntil` (`event.platform.context.waitUntil` on Cloudflare); it
never rejects.
