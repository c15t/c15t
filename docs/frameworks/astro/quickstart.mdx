...
For IAB policies, the browser fetches the vendor list from the backend's `/init`
endpoint after server rendering. That endpoint must be publicly reachable from
the app origin and allow its CORS requests without server-only credentials.
A private backend needs a public same-origin proxy that keeps authentication on
the server. Astro injects its local init route only in manifest mode; hosted mode
uses the configured backend URL directly.
