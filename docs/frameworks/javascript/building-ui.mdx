...
function createBanner() {
  ...
  banner.querySelector('#consent-customize')?.addEventListener('click', () => {
    consentStore.getState().setIsPrivacyDialogOpen(true);
  });
...
The store supports fine-grained subscriptions with a selector — only re-fire when the selected value changes:

```ts
// Only fires when showPopup changes
...
// Only fires when consents change
consentStore.subscribe(
  (state) => state.consents,
  (consents) => {
    console.log('Consent state updated:', consents);
  }
);

// Only fires when the privacy dialog opens/closes
consentStore.subscribe(
  (state) => state.isPrivacyDialogOpen,
  (isOpen) => {
...
| Property | Type | Use |
|----------|------|-----|
| `showPopup` | `boolean` | Whether to show the consent banner |
| `isPrivacyDialogOpen` | `boolean` | Whether the preference dialog is open |
...
| `setShowPopup` | `(show, force?) => void` | Show/hide the banner |
| `setIsPrivacyDialogOpen` | `(isOpen) => void` | Open/close the preference dialog |
...
Import base styles for consent UI elements:

```ts
import '@c15t/ui/styles/primitives';
```

This provides foundational CSS custom properties and reset styles that match the c15t design system.
