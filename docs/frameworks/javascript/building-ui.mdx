...
consentStore.subscribe((state, prevState) => {
  ...
  if (state.showPopup !== prevState.showPopup) {
    document.getElementById('consent-banner')!.style.display =
      state.showPopup ? 'block' : 'none';
  }
  ...
  if (state.isPrivacyDialogOpen !== prevState.isPrivacyDialogOpen) {
    document.getElementById('consent-dialog')!.style.display =
      state.isPrivacyDialogOpen ? 'block' : 'none';
  }
});
```
