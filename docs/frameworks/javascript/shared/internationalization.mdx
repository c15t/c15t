...
c15t also supports partial translations, which means you can adjust a single translation without having to pass the entire translations object. It will fallback to using the language, if the language is not supported, it will fallback to the default language.
