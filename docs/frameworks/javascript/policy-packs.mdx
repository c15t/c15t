...
If you prefer the nested form, this is equivalent:
...
|--------------|--------|
| `policyPacks` omitted | Synthetic opt-in fallback banner (local dev works out of the box) |
...
For production apps with a real backend, keep the canonical pack in backend `policyPacks` and treat the frontend pack as a preview aid.
