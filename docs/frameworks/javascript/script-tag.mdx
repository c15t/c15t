...
See [script gating](/docs/frameworks/javascript/script-loader) for the full `scripts` option, and the [integrations](/docs/integrations/overview) for ready-made GTM, GA4, and pixel loaders.
...
Cleanup is disabled unless configured. The configuration is initial-only; call `dispose()` and `init()` to replace it. Deleting data does not stop an already running tracker, so use the vendor's consent or shutdown API too. See [clear on revocation](/docs/integrations/clear-on-revocation) for prefix matching, cookie scopes, and browser limits.
...
The same events fire on `document` as `c15t:ready`, `c15t:consent`, `c15t:ui`, `c15t:surfaceShown`, and `c15t:error`, with the payload in `event.detail`. `surfaceShown` fires when the banner or the dialog becomes visible, with `{ surface, shownAt, snapshot }`; count it as an impression.
