...
| `data-policies` | Comma-separated policy presets for offline mode: `europeOptIn`, `europeIab`, `californiaOptIn`, `californiaOptOut`, `quebecOptIn`, `worldNoBanner`. Resolved by the visitor's country. |
...
Every method can be queued before the tag has loaded; `config` and `on` are the usual ones:
...
`window.c15t` also carries every field `useConsentManager()` returns in `@c15t/react`, under the same names: `consents`, `activeUI`, `model`, `policy`, `policyBanner`, `policyDialog`, `consentTypes`, `translationConfig`, `has()`, `hasConsented()`, `setActiveUI()`, `setConsent()`, `setSelectedConsent()`, `saveConsents('all' | 'necessary' | 'custom')`, and `subscribeToConsentChanges()`. A custom banner written against one moves to the other unchanged; the script-tag conveniences (`acceptAll()`, `openDialog()`, `on()`, `ready()`) sit on top.
...
Anything the page queues with `c15t.push(['config', ...])` before the tag still wins over the baked-in defaults. The response carries the same `Cache-Control` and `ETag` as `/manifest`, so put a CDN in front of it. Add `?language=de` to slice the manifest's translations to one language.
...
```html
<script src="https://cdn.jsdelivr.net/npm/@c15t/browser@0.1/dist/c15t.devtools.js" data-open data-tab="location" defer></script>
```

Tag order does not matter; the panel waits for the client. It is also on `window.c15t.devtools`, so `c15t.devtools.actions.setOverrides({ country: 'US' })` followed by `c15t.devtools.actions.init()` does the same from the console. Leave the tag out of production pages; it is 13 KB gzipped and shows internals. Bundler users import `mountDevTools` from `@c15t/browser/devtools`.
