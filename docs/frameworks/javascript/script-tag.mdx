...
<script src="https://cdn.jsdelivr.net/npm/@c15t/browser@0.1.0/dist/c15t.iab.js" defer></script>
...
Configure link destinations in `legalLinks`, then select which links each layer shows with `ui.banner.legalLinks` and `ui.dialog.legalLinks`, as in the script example above. An omitted or empty selection shows no links. The IAB banner always shows c15t branding, matching React. `ui.dialog.hideBranding` can hide it in the preference centre.
