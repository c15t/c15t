...
// UI state
state.showPopup          // true (banner should be visible)
state.isPrivacyDialogOpen // false
...
// Subscribe with a selector — only fires when the selected value changes
consentStore.subscribe(
  (state) => state.consents,
  (consents) => console.log('Consents changed:', consents)
);
