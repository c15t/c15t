...
Use a component inside `ConsentProvider` to update the current consent subject when Clerk's authentication state changes:

```tsx
import { useAuth } from '@clerk/react';
import { useIdentify } from 'c15t/react';
...
`identify()` replaces the subject's current `externalId`; it does not retain the anonymous ID as an alias. Once a visitor is linked to Clerk, clear the previous visitor ID from the application state or storage used by `getVisitorId()` so it cannot replace the Clerk ID later.
