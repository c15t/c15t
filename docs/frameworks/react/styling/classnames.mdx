...
Strip all default styles from a component and build from scratch:
...
When `noStyle` is true, components render with zero styling - no colors, no spacing, no layout. They're just semantic HTML elements that you style entirely yourself.
