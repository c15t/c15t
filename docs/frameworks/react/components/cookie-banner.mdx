...
The Cookie Banner supports a `layout` prop that controls button order and grouping. Buttons can be arranged individually or grouped together (typically displayed as a flex row).

The `primaryButton` prop determines which button(s) receive the primary styling variant (usually more prominent/colorful).

### Examples

```tsx
// Default: Reject and Accept grouped, Customize separate and primary
<CookieBanner 
  layout={[['reject', 'accept'], 'customize']} 
  primaryButton="customize" 
/>

// Customize first and primary, Reject and Accept grouped and both primary
<CookieBanner 
