...
|------|------|---------|-------------|
| `open` | `boolean` | — | Control the open state. If omitted, follows `isPrivacyDialogOpen` from context |
| `noStyle` | `boolean` | `false` | Remove all default styling |
| `disableAnimation` | `boolean` | `false` | Disable entrance/exit animations |
| `scrollLock` | `boolean` | `true` | Lock page scroll when dialog is visible |
| `trapFocus` | `boolean` | `true` | Trap keyboard focus within the dialog |
| `hideBranding` | `boolean` | `false` | Hide c15t branding in the footer |
| `showTrigger` | `boolean \| ConsentDialogTriggerProps` | `false` | Show a floating trigger button |
