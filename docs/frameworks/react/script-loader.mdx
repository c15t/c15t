...
Google Maps is SDK-backed, so `C15TGoogleMap` registers one shared Google Maps script through the c15t script manager and waits for the callback-shaped SDK readiness signal before creating each map instance.

YouTube is iframe-only, so `C15TYouTubeEmbed` builds on the existing `Frame` consent boundary instead of loading the YouTube iframe API. That keeps iframe media separate from SDK widgets and avoids a second source of truth for script readiness.
...
For iframe-only embeds, use the [iframe blocking](/docs/frameworks/react/iframe-blocking) pattern or `C15TYouTubeEmbed` instead of loading a JavaScript SDK to hide an iframe. For SDK-backed widgets, treat the SDK as a singleton and each rendered component as its own instance.
