...
For `YouTubeEmbed`, `className` and the forwarded ref target the iframe. Use `wrapperClassName` for the consent-gated `Frame`. Boolean values in `params` are serialized as YouTube's documented `1` and `0` values, and the iframe defaults to native lazy loading.

For custom SDK-backed widgets, call `useConsentScript()` with a c15t `Script` object. The hook uses the consent manager as the source of truth, shares compatible registrations inside that provider, and exposes a promise-shaped readiness result:

```tsx
const analytics = useConsentScript({
  script: {
    id: 'vendor-sdk',
