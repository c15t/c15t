...
export function CustomConsentBanner() {
  const { banner, performAction } = useHeadlessConsentUI();
          ...
          <div key={`${group.join('-')}-${index}`} className="flex gap-2">
            {group.map((action) => (
              <button
                ...
                onClick={() => void performAction(action, { surface: 'banner' })}
              >
                {action === 'accept'
...
export function CustomConsentDialog() {
  const { dialog, performDialogAction } = useHeadlessConsentUI();
  const {
    consentTypes,
    consentCategories,
    consents,
      ...
      <div className="mt-4 flex gap-2">
        {dialog.orderedActions.map((action) => (
          <button
            key={action}
            type="button"
            onClick={() => void performDialogAction(action)}
          >
            {action === 'accept'
              ? translations.common.acceptAll
              : action === 'reject'
