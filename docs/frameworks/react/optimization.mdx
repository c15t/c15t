...
| You are using Next.js and want SSR or static-route-specific guidance | Next.js optimization docs | Covers `C15tPrefetch`, `prefetchInitialConsent()`, and rendering tradeoffs | Next-specific |
...
Mount `ConsentProvider` as high in the tree as possible so route transitions do not recreate it.
...
- See [Next.js Server-Side Data Fetching](/docs/frameworks/next/server-side) for `prefetchInitialConsent()`
