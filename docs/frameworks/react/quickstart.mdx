...
<Steps>
  <Step>
    ...
    The single `c15t` package includes every framework entry point. Prefer installing the scoped packages individually? See [Granular Installs](/docs/granular-installs).
  </Step>
  ...
  <Step>
    ...
    export default function App() {
      return <ConsentManager>{/* your existing app */}</ConsentManager>;
    }
    ```
  </Step>
