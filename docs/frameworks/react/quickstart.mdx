...
<Steps>
  ...
  <Step>
    ### Create a `ConsentManager` component

    Create a component called `ConsentManager` and place the provider and consent UI in it. This initializes the consent store and makes consent state available to all child components.
...
<ConsentManagerProvider options={...}>
  <ConsentBanner />
  <ConsentDialog />
  {process.env.NODE_ENV !== 'production' && <C15TDevTools />}
  {children}
</ConsentManagerProvider>
```
