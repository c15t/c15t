...
|--------|------|----------|-------------|
| `backendURL` | `string` | Yes | Backend URL (absolute or relative path) |
| `headers` | `Headers` | Yes | The incoming request's `Headers` object |
| `overrides` | `Overrides` | No | Force specific country, region, or language |
| `debug` | `boolean` | No | Log SSR fetch details to server console |
