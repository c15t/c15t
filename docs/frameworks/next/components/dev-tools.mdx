...
For an App Router setup with server prefetching, put `DevTools` inside `ConsentProvider` and pass the server config through `options.prefetch`. Import the adapter from `c15t/next/devtools`, or `c15t/next/devtools` when you install the Next.js package directly.
