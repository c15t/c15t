...
A rule with `prompt: 'none'` still has a resolved model, so the preference
center and the dialog trigger stay available as the route to preferences.
With no resolved rule at all, because resolution failed, no rule matched and
you set no default, or init is still withheld, no consent surface renders:
not the banner, the dialog, the widget, the preferences link, or the trigger.
They appear as soon as a rule resolves, without a remount. `offline()`
without `policyRules` is that unconfigured state and shows nothing.
