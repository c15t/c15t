...
| `variant` | `PromptVariant` | `bar` on a notice, `floating` otherwise | Shape of the prompt. See [Variants](#variants). |
...
A floating card in a corner or centered on an edge. This is the default for a
choice prompt.
...
A bar across the full width of the viewport. From 1024px wide the text and
the controls share one row. This is the default for a notice.
...
`data-right`. The built-in stylesheet keys every variant's geometry on
`data-variant` and `data-position`, and uses `data-prompt="notice"` to lay
the footer out as one row with the rights links leading and dismiss trailing.
