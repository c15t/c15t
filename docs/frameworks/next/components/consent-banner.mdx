...
- A `notice` prompt shows an "Accept All" button and a neutral button for
  each right no action covers. Under an opt-out rule that is one button, "Do
  not sell or share my data". A notice never traps focus or locks scroll.
- A rule with `prompt: 'none'` renders nothing.

Right buttons open the preference center. An opt-out button therefore keeps
preferences reachable too, so a notice never shows a separate "Manage
preferences" button. Right buttons also appear on a choice prompt when nothing
else covers the right: a rule whose actions are only accept and reject renders
a "Manage preferences" button, because customize is absent.
...
every prompt. A notice keeps the same card, with its right button and "Accept
All" in the footer.
...
right buttons, and the controls share one row. Opt in to it for regions that
...
them, the right buttons. Use the individual parts when you need a different
...
- `ConsentBanner.Rights` renders one neutral button per uncovered right. It
...
- `ConsentBanner.RightLink` renders a single right. `right` is `'opt-out'`
  or `'preferences'`. By default it is a neutral button carrying
  `data-action="right"` and `data-right`; it opens the preference center on
  click and accepts `asChild` to render your own element, such as a link to a
...
Each action button carries `data-action`, and each right button carries
...
buttons leading and "Accept All" trailing.
