...
If the manifest request fails, or does not answer within `timeoutMs` (500 ms
by default), `resolveConsent` returns a baseline state without a resolved
policy and logs the failure outside production. The page renders without a
banner, optional categories stay denied and gated scripts and embeds stay
blocked. The browser resolves policy after hydration, so on that request a new
visitor sees the prompt appear late. Pass `onError` to report these failures;
see [troubleshooting](/docs/frameworks/next/troubleshooting).
...
leaves the page blank until the helper returns, for at most `timeoutMs`. A warm
manifest cache answers in about a millisecond; a cold cache pays the backend
round trip, and a request that misses the budget keeps running and fills the
cache for the next visitor. If the site cannot accept that wait, use the
[streaming form](#stream-the-page-while-consent-resolves), which renders the
shell immediately.
