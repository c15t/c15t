...
The init route is optional. This guide includes it so browser initialization
can resolve the cached manifest with request geography, without calling the
backend's `/init`. A successful server prefetch supplies the initial policy
directly. See [when to omit the init route](/docs/frameworks/next/api-reference/data-fetching#do-i-need-the-local-init-route).
