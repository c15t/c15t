...
If the app already has a global stylesheet, import the c15t stylesheet from
there instead of from the layout, after any Tailwind import, and drop the
`import 'c15t/next/styles.css'` line from `layout.tsx`:
...
Tailwind CSS 4 declares its cascade layers in that file. When the c15t
stylesheet is bundled separately and lands first, Tailwind's preflight sits
above the c15t component layer and strips the banner's padding, borders and
button styles.
...
`cacheComponents`, the boundary is still safe to keep. If Next.js reports an
