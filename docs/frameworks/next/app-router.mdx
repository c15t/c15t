...
`c15t/next/styles.css` holds the tokens and the banner, trigger and
`ConsentGate` rules. The dialog's rules come with the dialog's lazy chunk,
which Next.js loads after your global stylesheet, so they join the layer order
Tailwind already declared. See [Theme tokens and CSS](/docs/customization/tokens).
