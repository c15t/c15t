...
| `manifestURL` | `config.manifestURL` | Manifest route URL. When set, server prefetch resolves policy from the manifest and does not call backend `/init`. The manifest is read through the in-process manifest cache, whether the URL is your manifest route or the backend's `/manifest`. |
...
| `timeoutMs` | `500` | Longest the render waits for policy, in milliseconds, counted from the manifest or backend `/init` request. When it runs out, the helper returns the baseline state described below. `false` waits for the manifest cache's 5 second request timeout, or for the `fetch` implementation on backend `/init`. |
...
A `manifestURL` that points at the backend itself, rather than your manifest
route, is read through the same in-process cache, so renders do not fetch the
backend on every request. The manifest route adds the Next.js Data Cache and a
cacheable response for your CDN, so it remains the recommended setup.
