...
| `onBackgroundRevalidate` | none | Receives the promise of a background manifest refresh started by a request, so a host can keep it alive with `after` from `next/server` or a platform `waitUntil`. Called inside the handler; the promise never rejects. See [Optimization](/docs/frameworks/next/optimization#configure-manifest-cache-refresh). |
