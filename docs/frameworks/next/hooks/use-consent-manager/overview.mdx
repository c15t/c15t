...
<Callout type="warn">
  `ConsentProvider` runs on the client. Values passed through
...
Clerk can complete a sign-in without reloading the page. Add a client component inside `ConsentProvider` to update the current consent subject as soon as Clerk exposes the authenticated `userId`:
...
import { useIdentify } from 'c15t/next';
...
export function ClerkConsentSync() {
  ...
  }, [identify, isLoaded, isSignedIn, userId]);

  return null;
}
```

`identify()` replaces the subject's current `externalId`; it does not retain the anonymous ID as an alias. Once a visitor is linked to Clerk, stop supplying the former visitor ID. The example clears its client-readable cookie only after `identify()` succeeds so a later render cannot change the subject back to the anonymous ID.

<Callout type="info">
  Backend subject identification requires hosted mode (`mode: hosted({ url })`). In
  offline mode, `identify()` cannot update a backend subject. Identifiers
