...
`consentConfig` and manifest routes from the
[App Router guide](/docs/frameworks/next/app-router).

| Rendering path | Pass to `ConsentBoundary` | What the visitor receives |
| --- | --- | --- |
| Stream while consent resolves | The `prefetchInitialConsent` promise | The page can render while consent UI waits for the result. |
...
The App Router guide passes the promise from a synchronous layout:

```tsx
const initialConsent = prefetchInitialConsent({ config: consentConfig });

// In the existing layout, keeping its imports, children and consent UI:
...
Keep the shared config and both API routes from that guide. This form also
