...
<Callout type="info">
  Need fully static routes? Use `PrefetchC15T` in your layout and `getPrefetchedInitialData()` in your client provider instead of `fetchInitialData()`. See [Optimization](/docs/frameworks/next/optimization).
</Callout>
