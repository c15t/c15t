...
The awaited recipe inside `Suspense` builds and runs under
`cacheComponents: true` without extra calls; the repository's Next 16
compatibility suite covers that path. The error
`Next.js encountered the unstable value Date.now() while prerendering`,
pointing at the `await prefetchInitialConsent(...)` line, has been observed in
`next dev` when the manifest request fails, for example with an unreachable
`backendURL`: the failure path reads the clock before the request headers
have marked the component as dynamic. Fix the backend URL first. If the error
persists in your setup, opt the component into request time before the
helper runs and report the case on
...
outside the `Suspense` boundary still prerenders. It is not part of the
default recipe because the default recipe does not need it.
