...
Use the React renderable integrations through `@c15t/nextjs` in client components:

```tsx
'use client';

import { C15TGoogleMap, C15TYouTubeEmbed } from '@c15t/nextjs';
...
Google Maps is SDK-backed and uses one shared c15t-managed script plus per-component map instances. YouTube is iframe-only and uses the existing `Frame` boundary rather than the YouTube iframe API.

For iframe-only embeds, use the [iframe blocking](/docs/frameworks/next/iframe-blocking) pattern or `C15TYouTubeEmbed`. For SDK-backed widgets, load the SDK once and let each component instance create and clean up its own widget. Avoid mixing `next/script` with c15t for the same vendor.
