...
YouTube is iframe-only and uses the existing `Frame` boundary rather than the YouTube iframe API. Its `className` and forwarded ref target the iframe; `wrapperClassName` targets the `Frame`. Boolean `params` become `1` or `0`, and the iframe defaults to native lazy loading.
