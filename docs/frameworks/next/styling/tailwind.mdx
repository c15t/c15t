...
**Fix:** Keep `c15t/next/styles.css` at the end of the top-level `@import` block:
...
import { type ReactUIOptions, ConsentProvider } from 'c15t/next';
...
export function ConsentManager({ children }: { children: ReactNode }) {
  return (
    <ConsentProvider options={{ theme, mode: hosted({ url: '/api/c15t' }) }}>
      {children}
    </ConsentProvider>
  );
}
