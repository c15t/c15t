...
The local init route is optional. We include it so browser initialization can
resolve the cached manifest with request geography, without calling the
backend's `/init`. See [when to omit it](/docs/frameworks/next/api-reference/data-fetching#do-i-need-the-local-init-route).
