...
```tsx
import { useConsentManager, useTranslations } from 'c15t/next';
        ...
        {displayedTypes.map((type) => (
          <label key={type.name} className="flex items-center gap-3">
            <input
              type="checkbox"
              checked={
                selectedConsents[type.name] ?? consents[type.name] ?? false
              }
...
The headless UI still needs a `ConsentProvider`:
            ...
            >
              <div>
                <p className="font-medium">
                  {translations.consentTypes[type.name]?.title}
                </p>
              ...
              <input
                ...
                disabled={type.disabled}
                onChange={(e) =>
                  setSelectedConsent(type.name, e.target.checked)
                }
              />
            </div>
          ))}
