...
| `sessions` | `onReport` receives every session report: hosts' `POST /sessions` and the backend's own `/init`. The IP in its context follows `ipAddress`; a host report names the visitor only on `X-C15T-Client-IP`, and `/init` reads the connection. Reports land on the request's wide event. A failing sink never fails the request: on `POST /sessions` the failure is recorded on that event; on `/init` the sink runs after the response, registered with the runtime's `waitUntil` where one exists, and a failure is logged as its own event. Nothing is stored. Not accepted by `defineConfig`, like `observability`. |
