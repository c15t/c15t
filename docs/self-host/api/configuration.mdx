...
| `sessions` | `onReport` receives every session report: hosts' `POST /sessions` and the backend's own `/init`. The IP in its context follows `ipAddress`; a host report names the visitor only on `X-C15T-Client-IP`, and `/init` reads the connection. Failures are recorded on the request's wide event. Reports also land on that event. Nothing is stored. Not accepted by `defineConfig`, like `observability`. |
