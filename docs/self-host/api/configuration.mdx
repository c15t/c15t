...
| `sessions` | `onReport` receives every session report: hosts' `POST /sessions` and the backend's own `/init`. Runs detached from the response; the IP in its context follows `ipAddress`. Reports also land on the request's wide event. Nothing is stored. |
