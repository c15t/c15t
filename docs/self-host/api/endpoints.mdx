...
prerender (`Sec-Purpose`, `Purpose`, `Next-Router-Prefetch`), nor a `HEAD` probe.
Each report is one resolution, not one visitor. A page view can produce more
than one, for example a `render` followed by a `route` when the client
re-initialises, and every full page load produces a new one. The report carries
no visitor identifier and nothing ties a `render` to the `route` from the same
page view, so a consumer that wants sessions builds them: group reports by the
forwarded address and user agent within a window, and count a visitor once per
window. That count is approximate by design.
