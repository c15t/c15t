...
manifest, and returns the map of the most recent act as `subjectVendorChoice`. A
map that omits a vendor the manifest declares is stored too: the client saw an
older manifest, and the omitted vendor reads back as never decided and follows
its category, the same as a vendor added after the act.
