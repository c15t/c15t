...
| `POST /sessions` | Report an init a host resolved from a cached manifest. Nothing is stored. | No |
...
report server-to-server after each resolution, detached from the response, and
forward the visitor's IP chain and user agent as request headers. The backend
answers `204`, records the report on the request's wide event and hands it to
the `sessions.onReport` sink. It writes nothing to the database. The backend's
own `/init` emits the same event. The body shape is `consentSessionReportSchema`
in `@c15t/schema`; `source` is `route` for an init route, `render` for a
server-rendered page and `init` for the backend's own route, so a consumer can
dedupe a render followed by a client re-init.
