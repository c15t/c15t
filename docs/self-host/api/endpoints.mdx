...
the consent row and returns the newest map as `subjectVendorChoice`. A grant for a
vendor the manifest does not declare is refused with `VENDOR_OUT_OF_SCOPE`; a denial
for one is kept. Retrying the same act with a different map is a `CONFLICT`, like a
retry with different receipts. Migration `4-vendor-choice` adds the column.
