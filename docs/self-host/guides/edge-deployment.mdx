...
The `/init` endpoint determines consent policy from geo headers or request context, resolves translations, and optionally fetches the GVL. None of this requires a database. The `@c15t/backend/edge` export lets you run this logic in edge runtimes (Vercel Middleware, Cloudflare Workers, Deno Deploy, Netlify Edge Functions) so the consent banner resolves from the nearest PoP instead of round-tripping to your origin.
...
<Tabs groupId="runtime" items={['Vercel Middleware', 'Cloudflare Workers', 'Deno Deploy', 'Netlify Edge Functions']}>
  ...
  <Tab value="Netlify Edge Functions">
    ```ts title="netlify/edge-functions/c15t-init.ts"
    import { unstable_c15tEdgeInit } from '@c15t/backend/edge';
    import { consentConfig } from '../../lib/consent-config';

    export default unstable_c15tEdgeInit(consentConfig);
    ...
    Netlify Edge Functions do not add geo request headers. Exporting the handler directly lets Netlify pass `context` as the second argument so c15t can read `context.geo`.
  </Tab>
</Tabs>
...
1. **Reads location** — from provider headers or from request context. Vercel sets `x-vercel-ip-country` and `x-vercel-ip-country-region`. Cloudflare sets `cf-ipcountry`. Netlify Functions set `x-country` (country only) and pass `context.geo` with country and subdivision. Netlify Edge Functions do not inject geo headers; they pass `context.geo` as the second handler argument.
