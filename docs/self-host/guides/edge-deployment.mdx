...
Edge:      Browser → Edge   (nearest PoP)    → edgeInitHandler     → Response
```
...
import type { C15TEdgeInitOptions } from '@c15t/backend/edge';
...
const initHandler = createEdgeInitHandler(consentConfig);
...
`createEdgeInitHandler` accepts `C15TEdgeInitOptions` — the same fields as `c15tInstance` minus the database-related options (`adapter`, `tablePrefix`, `basePath`, `openapi`, `ipAddress`, `apiKeys`, `background`).
