...
Serverless function requests also include `x-country` with a two-letter country code. That header is country only. California (CCPA) and Quebec (Law 25) still need the `context.geo` subdivision. Do not copy `x-nf-geo`; it is not a public API.
