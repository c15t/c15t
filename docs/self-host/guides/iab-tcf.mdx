...
If you self-host and have your own CMP registration with IAB Europe, configure your CMP ID via `advanced.iab.cmpId`. This value is returned to clients in the `/init` response so they use the correct CMP identity in TC Strings.
...
<Callout type="warn">
  If you heavily customize or build your own IAB banner or dialog (rather than using the default `IABConsentBanner` and `IABConsentDialog` components provided by c15t), you cannot use consent.io's CMP ID. You must register your own CMP with [IAB Europe](https://iabeurope.eu/tcf-2-0/) and configure your CMP ID via `advanced.iab.cmpId`.
</Callout>
