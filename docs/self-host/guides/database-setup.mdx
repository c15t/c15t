...
Kysely migrations run automatically on first request — no manual migration step needed.
...
Generate the Drizzle schema and include it in your existing migration workflow.
...
Add the c15t models to your `schema.prisma` and run `prisma migrate dev`.
...
MongoDB collections are created automatically on first request.
...
<Callout type="info">
  All tables include an optional `tenantId` column for multi-tenant deployments. Set `tenantId` in your `c15tInstance()` config to automatically scope all queries to a single tenant.
</Callout>
