...
If you are enabling policy-pack runtime audit storage, also apply the runtime policy decision migration described in the [Policy Packs guide](/docs/self-host/guides/policy-packs). That migration adds the `runtimePolicyDecision` table plus consent linkage columns used by signed snapshots and write-time audit records.
...
| `runtimePolicyDecision` | Resolved runtime policy evidence linked from consent rows when policy packs are active. |
