...
Policy-pack audit storage adds runtime decision evidence to the database:

- `runtimePolicyDecision` table
- `consent.runtimePolicyDecisionId`
- `consent.runtimePolicySource`

Apply the required database changes before enabling policy-pack audit persistence in production. The example SQL migration lives in the repo at `examples/demo/migrations/2026-03-02-runtime-policies.sql`.

If you use table prefixes or a non-default schema, adjust the identifiers before running it.

Without these columns, the backend cannot persist runtime policy decision evidence for consent writes.
