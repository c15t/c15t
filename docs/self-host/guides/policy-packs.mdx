...
<Tabs items={['Simplified', 'Sequence Diagram']}>
  <Tab value="Simplified">
    ...
    2. **Resolve policy** — walks the pack in priority order: region → country → default
...
3. **Default** — fallback for unmatched traffic
...
- Policies with no matchers and not marked as default
