...
`fallbackLanguage` controls which language inside the active profile is used
...
`zh`. If the browser asks for `zh`, c15t falls back to `fallbackLanguage`
inside the `eu` profile, which would be `en` in this example.
