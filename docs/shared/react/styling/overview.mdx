...
<section id="intro">
  ...
  3. **Component slots** — targeted attributes for specific parts such as the banner card, footer, or title
  ...
  - If you are still using the stock banner, dialog, or widget, stay with props, tokens, and component slots.
  ...
  | **Component slots** | Medium | Targeting specific component parts (for example `banner.footer` or `dialog.card`) |
  | **CSS variables / className** | Medium | Integrating with an existing stylesheet or utility classes after tokens and component slots |
...
<section id="styling-paths-to-api">
  ...
  - `i18n` on `ConsentProvider` for copy changes
  ...
  Target specific component parts via the provider `components` object:
  ...
  Use component slots when the component part is right but the local styling needs adjustment.

  ### 4. CSS variables and className-level overrides

  Override `--c15t-*` custom properties in your stylesheet or attach classes through component slots when your app styling is driven externally.

  Reach for this after tokens and component slots, not before.

  ```tsx
  options={{
    components: {
      banner: {
        footer: { className: 'bg-[var(--banner-footer)]' },
      },
    },
  }}
  ```
  ...
  options={{
    components: {
      banner: {
        ...
        title: { className: 'text-xl tracking-tight' },
      },
    },
  }}
  ```
  ...
  <Callout type="warning">
    ...
    - Use either tokens/component slots or raw CSS variable overrides intentionally to avoid conflicting style sources.
    - For dark mode, c15t supports `.dark` and `.c15t-dark`.
  </Callout>

  ## API Reference

  <AutoTypeTable path="./packages/react/src/types/theme/style-types.ts" name="Theme" />
</section>
