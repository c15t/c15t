...
<section id="behavior">
  ...
  ends with a "Vendors" group: one row per vendor whose category condition
  names that category, with the vendor's name, description, a privacy policy
  link and a switch. Vendor switches edit the same draft as the category
  ...
  hint "Turn on this category to choose vendors." Accept all and Reject all
  clear every vendor denial. Under an `iab` policy no vendor rows render.
  Vendors that are only referenced by a script slug, without a name and
  privacy policy URL, still gate loading but are not listed. See
  [vendor-level consent](/docs/integrations/vendor-consent).
...
<section id="accessibility">
  ...
  name. Vendor rows form a `group` labelled "Vendors"; each vendor switch is
  named "Allow" followed by the vendor name, and described by the vendor's name element. The policy-change alert uses `role="alert"`, and a restriction note is
...
<section id="composition">
  ...
  `vendor-list.root`, `vendor-list.item`, `vendor-list.name`,
