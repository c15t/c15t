...
<section id="customization">
  ...
  <Callout type="info">
    Most widget customization should stay in the stock component. Use theme tokens and component slots such as `accordion.root`, `manager.footer`, and `manager.actionGroup` before reaching for compound components. See [Styling Overview](/docs/frameworks/react/styling/overview).
  </Callout>
  ...
  Use `manager.footer` for styling the whole action area, such as border and padding. Use `manager.actionGroup` for spacing or alignment inside nested button groups.

  Widget copy should be changed through `ConsentProvider.options.i18n` so the inline UI stays aligned with the rest of the consent experience.
  ...
  If the stock widget structure is already correct, stay with tokens and component slots instead of rebuilding the layout.
</section>
