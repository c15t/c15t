...
<section id="intro">
  `ConsentDialog` is a modal that shows toggles for each consent category. In **opt-in jurisdictions**, it typically opens when users click "Customize" on `ConsentBanner`. It can also be controlled programmatically for use on settings pages regardless of jurisdiction. The default dialog connects its title and description to the modal so screen readers announce both when it opens.
</section>
...
<section id="customization">
  ...
  <Callout type="info">
    If you are only changing visuals, stay with the stock dialog and use the theme system first. Start with tokens and component slots such as `dialog.card`, `manager.footer`, and `tag.dialog`. See [Styling Overview](/docs/frameworks/react/styling/overview).
  </Callout>
  ...
  Dialog copy should be changed through `ConsentProvider.options.i18n`, not by rebuilding the dialog structure.
  ...
  If the stock dialog structure still works, prefer tokens, component slots, and provider configuration instead.
</section>
