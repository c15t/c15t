...
<section id="policy-packs">
  ...
  When you deliberately choose offline mode, `ConsentManagerProvider` accepts `offlinePolicy.policyPacks` for local policy resolution during development, testing, previews, or deployments without a backend:
  ...
  - Treat offline policies as a development or testing tool, or as the policy source for a deployment that deliberately runs without a backend.
  - `offlinePolicy.i18n` lets offline mode mirror hosted `messageProfile` and profile-local `fallbackLanguage` behavior.
  - Omitting `offlinePolicy.policyPacks` uses the built-in synthetic opt-in policy in offline mode. Hosted init failures keep the banner hidden while c15t retries.
