...
<section id="policy-packs">
  ...
  In `offline` mode, `ConsentManagerProvider` accepts `offlinePolicy.policies` for local policy resolution.
  ...
  - Omitting `offlinePolicy.policies` uses the built-in synthetic opt-in fallback banner.
  - `offlinePolicy: { policies: [] }` is explicit no-banner mode.
