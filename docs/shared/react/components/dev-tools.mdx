...
<section id="customization">
  ...
  saves preserve the accept/reject action in the transport payload. Without an
  ...
  Keep script-loader debug events enabled to capture lifecycle results.
  Setting `emitToV2DebugListeners` to `false` without `onDebug` can leave load results
  unobserved. The page scan lists external scripts and iframes present in the
  ...
  `@c15t/core/v3/modules/script-loader`. The subscription reports loader
  ...
  The IAB panel connects to the existing `@c15t/iab/v3` module attached to the
  ...
  `subscribeIABControls(kernel, listener)` are exported from `@c15t/core/v3`.
