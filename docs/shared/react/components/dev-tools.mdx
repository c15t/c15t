...
<section id="intro">
  `DevTools` is a floating panel for the c15t v3 kernel. Use it during development to inspect explicit choices, effective permissions, scripts, location, policy resolution, IAB state, events, and consent actions. The framework adapter reads the kernel from the nearest v3 provider. It never discovers a store through a window global.
...
<section id="customization">
  ...
  hidden consent values unchanged. `necessary` always remains enabled. Framework adapters use their provider's displayed categories. Direct kernel
  integrations use the current policy categories by default.
  ...
  explicit scope, bulk saves confirm the current policy scope. They never
  override GPC or standing privacy restrictions, and a category save does not
  dismiss a required notice. Notice dismissal is a separate user action.
  ...
  | **Policy** | Resolution outcome, active rule, prompt requirement and policy rights |
  ...
  Category toggles stage a DevTools draft. They do not grant permissions until
  Save confirms the selections. The Actions panel can dismiss the current local
  notice through the public notice command without recording category consent.
  Framework adapters bind record clearing to the provider's existing persistence
  instance. With persistence disabled, clearing affects only in-memory records.
  Direct kernel integrations supply their existing persistence handle's `clear`
  callback.

  The Policy panel labels presentation diagnostics as resolved defaults unless
  host presentation options are available. Framework adapters read those options
  from the provider; direct integrations supply `getPresentation`. It resolves the supplied
  host configuration; it does not verify the controls rendered by the app. See
  the [imperative API](/docs/frameworks/javascript/dev-tools) for draft,
  presentation, and persistence callbacks.
  ...
  `c15t/modules/script-loader`. With a scoped framework package, install
  `@c15t/core` as a direct dependency and import from
  `@c15t/core/modules/script-loader` instead. The subscription reports loader
  ...
  Toggles edit IAB values without establishing new confirmed processing
  authority. Use Save IAB consent to generate a fresh TC string, record the
  current authority and run the configured save transport. Processing gates
  read confirmed TCF authority and current restrictions. The panel displays
  the last generated string; unsaved edits are not represented in it. Copy TC string confirms success or reports clipboard failures. Raw IAB
  ...
  With a scoped framework package, install `@c15t/core` directly and import
  these helpers from `@c15t/core` instead.
