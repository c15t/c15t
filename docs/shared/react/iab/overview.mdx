...
<section id="prepare">
  ...
  A CMP ID is assigned to a company when it registers as a CMP with IAB Europe
  ([register.consensu.org/CMP](https://register.consensu.org/CMP)). Every TC
  String names the CMP that wrote it, so an ID you did not register attributes
  your users' consent records to someone else. Omitting it leaves the add-on
  unmounted: no `__tcfapi`, and no TC String written.
