...
<section id="config-and-concepts">
  ...
  Configure IAB mode with `iab({ ... })` from `@c15t/iab`. The factory enables the addon, and the framework provider loads the runtime module when needed. The user-facing options are:
