...
<section id="intro">
  ...
  <Callout type="warning">
    Headless is the last step in the customization ladder. Use this guide only when pre-built components, tokens, component slots, compound components, and `noStyle` are no longer sufficient.
  </Callout>
  ...
  - `resolveConsentPresentation` from `@c15t/core` for the pure presentation resolver that framework packages build on
  ...
  - `offline({ policyRules })` for offline previews that behave like backend policy resolution

  The split is intentional: `@c15t/core` owns pure presentation resolution, while the framework hooks own visibility, consent mutations, and reactive state.
...
<section id="before-you-build">
  ...
  - Use component slots such as `banner.card`, `banner.footer`, and `dialog.card` before compound components
  - Use `ConsentProvider.options.i18n` before rebuilding UI just to change text
...
<section id="not-for-styling">
  ...
  Those should stay in the pre-built stack with tokens, component slots, `theme.consentActions`, and provider `i18n`.
</section>
...
<section id="checklist">
  ...
  - `resolution` when you want to debug why a specific UI state was chosen
