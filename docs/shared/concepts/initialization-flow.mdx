  ...
  <Tabs items={['Simplified', 'Sequence Diagram']}>
    ...
    <Tab value="Sequence Diagram">
      ...
      sequenceDiagram
          ...
          Store->>Store: Set activeUI, auto-grant if opt-out/null
...
<section id="how-it-works">
  ...
  **Model resolution** — The jurisdiction from the init response is mapped to a consent model: `opt-in` (GDPR and similar), `opt-out` (CCPA), `iab` (TCF 2.3), or `null` (no regulation). For `opt-out` and `null` models, all categories are auto-granted — unless the resolved policy has `consent.gpc: true` and the browser sends a Global Privacy Control signal, in which case `marketing` and `measurement` are denied. See [Consent Models](/docs/frameworks/react/concepts/consent-models) for the full jurisdiction-to-model mapping.
...
<section id="debugging">
  ...
  | Banner visibility | Consents | — | `activeUI` in store state; `consentInfo` null? |
