...
<section id="overview">
  ...
  3. **Offline fallback** — pass the same policy shapes to the frontend via `offlinePolicy.policyPacks`. Used as a resilience fallback when the backend is unreachable, or for quick local experimentation and demos.
...
<section id="quickstart">
  ...
  |--------|-------|----|---------|
  | `europeOptIn()` | `opt-in` | banner | EEA + UK countries |
  | `europeIab()` | `iab` | banner | EEA + UK countries (TCF 2.3) |
...
<section id="resolution-flow">
  ...
  3. **Fall back to default** — if nothing matches, uses the policy marked as the default
  4. **No match, no default** — resolves to no-banner mode (silent, no consent UI)
