...
<section id="overview">
  ...
  3. **Offline fallback** — pass the same policy shapes to the frontend via `offlinePolicy.policies`. Used as a resilience fallback when the backend is unreachable, or for quick local experimentation and demos.
