...
<section id="hosted-mode">
  ...
  <Callout type="info">
    Use `hosted({ url })` for backend-backed consent. The legacy string modes
    were removed in v3.
  </Callout>
  ...
  **What happens:**
  ...
  3. If init fails, the banner stays hidden and gated resources stay blocked while c15t retries. Failed consent saves are queued and replayed later.
  ...
  **Why it is the default for production:**
  ...
  - If the backend is temporarily unavailable, c15t fails closed until it can load the authoritative policy
...
<section id="offline-mode">
  ...
  **Best for:** Local development, Storybook or static demos, deployments without a backend, or simpler sites that explicitly accept browser-only consent storage.
</section>
...
<section id="custom-mode">
  ...
  **What happens:**
  ...
  2. When the user grants or changes consent, it is saved locally before being synced to the backend.
  3. If one of your handlers fails, c15t keeps local consent state and uses the same init retry and failed-save replay behavior as the hosted transport
