...
The default mode. Connects to a c15t backendfor full consent lifecycle management. We recommend using [consent.io](https://consent.io) for a fully managed experience however you can [self-host](/docs/self-host) as well.
...
3. If the backend is unreachable, it falls back to Offline mode and resync's with the backend when it's available
