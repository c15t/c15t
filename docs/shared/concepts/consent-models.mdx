...
This is the model applied to jurisdictions including the EU (GDPR), the UK (UK GDPR), Switzerland, Brazil (LGPD), Japan (APPI), South Korea (PIPA), and Canada (PIPEDA). It is also the fallback model if c15t does not know the vistors location - the library defaults to the strictest behavior to minimize compliance risk.
...
Returned when no jurisdiction has been detected. This occurs in several situations: geolocation has not yet resolved (the API call is still in flight), the user's location could not be determined (network error, blocked request), or the jurisdiction was explicitly set to `null` or `NONE`.

In this state, no consent banner is displayed. Consent categories remain in their initial default state. Once the jurisdiction resolves, the model updates automatically and the appropriate banner behavior activates. This transient state ensures that the UI does not flash a consent banner before the library knows which model to apply.
</section>
