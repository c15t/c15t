...
<section id="what-sets-cookies">
  ...
  **1. c15t itself**
  c15t stores the user's explicit choice in a `c15t` cookie and mirrors it to localStorage. The record holds one receipt per optional category: the value, the time that category was confirmed, and the policy contract it was confirmed under. It also holds the subject ID once. This is a strictly necessary cookie - it exists so the site remembers the user's choice. Loading the stored choice at startup is read-only: it does not rewrite the cookie, renew a receipt, or migrate an older record. A valid record written by c15t v2 keeps working; it is read in memory and rewritten in the v3 format only by the next explicit accept, reject, or save. Only those explicit actions write the choice record, and each one refreshes exactly the categories it confirmed.

  Two smaller local records live next to it under the same storage key with a suffix: `c15t-notice` holds the dismissal of the current notice, and `c15t-privacy` holds standing opt-out directives recorded from a Global Privacy Control signal. Both are written to localStorage and to a compact cookie so a server render can read them from the request. Neither changes the choice record, and a missing or malformed one never changes permissions. Clearing c15t data removes all three records, their cookies, and any queued backend replay.
