...
c15t gates the Hotjar script from loading until `measurement` consent is granted. Your application code that calls Hotjar's runtime API (`window.hj(...)`) is **not** automatically gated - `window.hj` does not exist until the script is loaded, so unguarded calls before consent throw.
