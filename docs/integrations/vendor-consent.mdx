...
the visitor never saw cannot have been turned off.
...
The React `ConsentWidget` and `ConsentDialog` list vendors under their
category when the provider declares them. Pass the same `vendors` array to
`ConsentProvider`, or to `ConsentRoot` in Next.js and TanStack Start:
...
Each category row expands to a "Vendors" group with a switch per vendor. The
switches disable while the category is off in the draft, and Save records the
vendors the visitor moved. Copy comes from
`consentManagerDialog.vendors.{title, privacyPolicy, disabledByCategory,
switchLabel}` in the translations. The Vue and Svelte preference centers do
not render vendor rows yet; they still gate scripts, requests and iframes.
