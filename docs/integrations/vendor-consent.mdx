...
manifest slug, so `googleTag()` and `metaPixel()` need no extra wiring.
...
same id, the copy declared in code wins. A slug used by a script but declared
nowhere still gates loading. It is hidden from the preference surface, and the
...
both clear that list, so vendors follow their category on a bulk action. A
...
cookies that vendor already set. Configure
[clear on revocation](/docs/integrations/clear-on-revocation) for that, keyed by
