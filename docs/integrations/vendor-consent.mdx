...
Add `vendors` to your runtime options, or to the React provider's options.
Each vendor sits inside a category condition and needs a name and a privacy
policy URL so the preference surface can list it. Scripts, rules and iframes
are gated the same way whichever place declares it:
