...
`ConsentProvider`, or to the `vendors` prop of `ConsentRoot` in
[Next.js](/docs/frameworks/next/script-loader#granular-consent) and
[TanStack Start](/docs/frameworks/tanstack-start/quickstart#granular-consent):
...
`@c15t/react` to read and stage the same grants, and `useVendorAllowed(id)` to
