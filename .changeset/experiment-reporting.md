...
The `'posthog'` target holds events while `window.posthog` is not on the page yet (a consent-gated load, for example) and sends them in order once it appears. Each `createExperimentReporting` subscription owns its own buffer and poll timer, and its disposer clears both, so a disposed runtime never reports late; `createPosthogReporter` is exported for custom sinks that want the same buffering. An `onError` handler that throws is swallowed so the remaining reporters still receive the event.
