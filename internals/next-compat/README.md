...
| --- | --- | --- | --- | --- |
| `next-15-app` | 15 | App | webpack | client, prefetch, ssr, isr, manifest, manifest-ssr |
...
- `prefetch`: `C15tPrefetch` starts `/init` before hydration and the runtime adopts that promise instead of fetching again.
...
3. Who called `/init` and `/manifest`. The backend stub records every request; browser requests carry `sec-fetch-site`, server requests do not. `ssr` needs a server `/init` call with `x-c15t-country`, `client` and `prefetch` need exactly one browser call and no server call, the manifest scenarios need zero `/init` calls, and `manifest` additionally needs the backend `/manifest` to be fetched once across two page loads (the caching claim).
4. The first HTML contains the banner only for `ssr`, and the prefetch script only for `prefetch`.
...
- `C15tPrefetch` uses `beforeInteractive`, which Next.js only honours in a root layout (App Router) or `_document` (Pages Router). The App Router cells keep prefetch routes under a route group with its own root layout for that reason.
- In `pages/_document`, Next only registers `beforeInteractive` scripts that are direct `next/script` children of `<Head>` or `<body>`, so a wrapper component such as `C15tPrefetch` renders nothing there. The Pages cells put `buildPrefetchScript()` inside a bare `<Script>`.
...
- `prefetchInitialConsent` reads request headers and cookies, so its route is dynamic. Static and ISR routes use `C15tPrefetch` instead.
