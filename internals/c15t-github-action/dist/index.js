...
async function findPreviousComment(octokit, repo, number, header, authorLogin) {
    ...
    while(hasNextPage){
      ...
      `, {
        ...
        const normalizeLogin = (login)=>(login ?? '').replace('[bot]', '').trim().toLowerCase();
        const expectedLogin = normalizeLogin(authorLogin ?? viewer.login);
        const target = repository.pullRequest?.comments?.nodes?.find((node)=>normalizeLogin(node?.author?.login) === expectedLogin && !node?.isMinimized && node?.body?.includes(h));
...
async function updateComment(octokit, id, body, header, previousBody) {
    if (!body && !previousBody) return core_namespaceObject.warning('Comment body cannot be blank');
    ...
    `, {
        input: {
            id,
            body: previousBody ? bodyWithHeader(`${rawPreviousBody}\n${body}`, header) : bodyWithHeader(body, header)
        }
    });
}
async function createComment(octokit, repo, issue_number, body, header, previousBody) {
    if (!body && !previousBody) return void core_namespaceObject.warning('Comment body cannot be blank');
...
const pullRequestNumber = github_namespaceObject.context?.payload?.pull_request?.number || +core_namespaceObject.getInput('number', {
    required: false
});
...
const config_hideDetails = core_namespaceObject.getBooleanInput('hide_details', {
    required: true
});
const recreate = core_namespaceObject.getBooleanInput('recreate', {
    required: true
});
const hideAndRecreate = core_namespaceObject.getBooleanInput('hide_and_recreate', {
    required: true
});
...
const config_authorLogin = core_namespaceObject.getInput('author_login', {
    required: false
}) || 'consentdotio';
function buildRepo() {
    return {
        owner: core_namespaceObject.getInput('owner', {
            required: false
        }) || github_namespaceObject.context.repo.owner,
        repo: core_namespaceObject.getInput('repo', {
            required: false
...
async function getBody() {
    ...
    const followSymbolicLinks = core_namespaceObject.getBooleanInput('follow_symbolic_links', {
        required: true
    });
    if (!pathInput || !(pathInput.length > 0)) return core_namespaceObject.getInput('message', {
        required: false
    });
    try {
        const globber = await (0, glob_namespaceObject.create)(pathInput.join('\n'), {
            followSymbolicLinks,
            matchDirectories: false
        });
        return (await globber.glob()).map((path)=>(0, external_node_fs_namespaceObject.readFileSync)(path, 'utf-8')).join('\n');
    } catch (error) {
...
async function run() {
    if (Number.isNaN(pullRequestNumber) || pullRequestNumber < 1) return void core_namespaceObject.info('no pull request numbers given: skip step');
    try {
        const body = await getBody();
        if (!body && ignoreEmpty) return void core_namespaceObject.info('no body given: skip step by ignoreEmpty');
        if (!deleteOldComment && !hideOldComment && !body) throw new Error('Either message or path input is required');
        if (deleteOldComment && recreate) throw new Error('delete and recreate cannot be both set to true');
        if (onlyCreateComment && onlyUpdateComment) throw new Error('only_create and only_update cannot be both set to true');
        if (hideOldComment && hideAndRecreate) throw new Error('hide and hide_and_recreate cannot be both set to true');
        const octokit = github_namespaceObject.getOctokit(githubToken);
        const previous = await findPreviousComment(octokit, config_repo, pullRequestNumber, config_header, config_authorLogin || void 0);
        ...
        if (!previous) {
            ...
            const created = await createComment(octokit, config_repo, pullRequestNumber, body, config_header);
            core_namespaceObject.setOutput('created_comment_id', created?.data.id);
            return;
        }
        if (onlyCreateComment) return;
        if (hideOldComment) return void await minimizeComment(octokit, previous.id, hideClassify);
        if (skipUnchanged && commentsEqual(body, previous.body || '', config_header)) return;
        const previousBody = getBodyOf({
            body: previous.body || ''
        }, config_append, config_hideDetails);
        if (recreate) {
            await deleteComment(octokit, previous.id);
            const created = await createComment(octokit, config_repo, pullRequestNumber, body, config_header, previousBody);
            core_namespaceObject.setOutput('created_comment_id', created?.data.id);
            return;
        }
        if (hideAndRecreate) {
        ...
        await updateComment(octokit, previous.id, body, config_header, previousBody);
    } catch (error) {
        if (error instanceof Error) core_namespaceObject.setFailed(error.message);
    }
}
